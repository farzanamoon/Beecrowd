...
void view_list()
{
    ...
    while(fscanf(view,"%d %s %s %d %d/%d/%d %d %s %s %lf %s %f %d/%d/%d",&add.acc_no,&add.cl_pass,&add.name,&add.n_id,&add.dob.month,&add.dob.day,&add.dob.year,&add.age,add.address,add.citizenship,&add.phone,add.acc_type,&add.amt,&add.deposit.month,&add.deposit.day,&add.deposit.year)!=EOF)
       {
...
void see(void)
{
    ...
    {   printf("Enter the name:");
        ...
        while (fscanf(ptr,"%d %s %s %d %d/%d/%d %d %s %s %lf %s %f %d/%d/%d",&add.acc_no,&add.cl_pass,&add.name,&add.n_id,&add.dob.month,&add.dob.day,&add.dob.year,&add.age,add.address,add.citizenship,&add.phone,add.acc_type,&add.amt,&add.deposit.month,&add.deposit.day,&add.deposit.year)!=EOF)
        {
            ...
            {   system("cls");
                test=1;
                printf("\nAccount No.:%d\n passward:%s\nName:%s \nNID:%d\nDOB:%d/%d/%d \nAge:%d \nAddress:%s \nCitizenship No:%s \nPhone number:%.0lf \nType Of Account:%s \nAmount deposited:$%.2f \nDate Of Deposit:%d/%d/%d\n\n",add.acc_no,add.cl_pass,add.name,add.n_id,add.dob.month,add.dob.day,add.dob.year,add.age,add.address,add.citizenship,add.phone,
                add.acc_type,add.amt,add.deposit.month,add.deposit.day,add.deposit.year);
                if(strcmpi(add.acc_type,"fixed1")==0)
                    {
                        time=1.0;
